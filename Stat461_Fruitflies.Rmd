...
In general, a group of fruit flies will live, on average, approximately r round(dummy.coef(fruitflies_Model)[[1]], 2) days; this is our estimate for baseline longevity ($\widehat{\mu_{\bullet\bullet}}$).. We can also see the factor level (treatment) effects ($\widehat{\alpha_i}$) estimates. For males exposed to eight virgin females per day exhibited a reduction in longevity by `r round(dummy.coef(fruitflies_Model)$condition[6], 2)`days, whereas males exposed to one virgin female per day showed a decrease of `r round(dummy.coef(fruitflies_Model)$condition[3], 2)` days. Males exposed to one pregnant female per day experienced a reduction of `r round(dummy.coef(fruitflies_Model)$condition[4], 2)` days. In contrast, males with no female exposure, serving as our baseline control, had a point estimate effect of `r round(dummy.coef(fruitflies_Model)$condition[2], 2)`days, and males exposed to eight pregnant females per day had a reduced longevity of `r round(dummy.coef(fruitflies_Model)$condition[5], 2)` days. This analysis indicates that males in the Eight Virgin and Eight Preg conditions experienced a greater reduction in longevity compared to the baseline (*GSAM*).
