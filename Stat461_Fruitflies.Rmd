...
fruitflies %>%
  drop_na(condition) %>%
  ggplot(mapping = aes(x = condition, y = longevity, color = condition, shape = condition)) +
  geom_point(na.rm = TRUE) +
  theme_bw() +
  labs(x = "Condition", y = "Longevity") +
  scale_color_manual(values = c("blue", "red", "green", "purple", "orange")) + # Make sure to adjust the colors accordingly
...
```{r}
# Basic boxplot
boxplot(longevity ~ condition, data = fruitflies, main="Longevity by Condition", xlab="Condition", ylab="Longevity")

# Adding the Tukey post hoc groupings
letters <- multcompView::multcompLetters4(
  object = fruitflies_Model, 
  comp = hsdfruitflies, 
  threshold = 0.1
)$condition

# Compute boxplot statistics for letter placement
box_stats <- boxplot.stats(fruitflies$longevity[fruitflies$condition == levels(fruitflies$condition)[1]])$stats

# Place the letters above the boxplot
text(1:length(levels(fruitflies$condition)), box_stats[5] + 1, labels = letters, cex = 1.5, col = "red")
