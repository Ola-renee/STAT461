...
bechdel_awards %>%
  ...
  ggplot(mapping = aes(x = ≈, fill = awards_factor)) +
...
plot(imbd_model$fitted.values, imbd_model$residuals)
abline(h = 0, col = "red")
```


```{r indexplot}
#| fig.cap = "Index plot of  Residual",
#| fig.height = 2,
#| fig.width = 4,
#| fig.pos = "H"
...
data = data.frame(
residuals = imbd_model$residuals,
index = 1:length(imbd_model$residuals)
),
...
car::durbinWatsonTest(imbd_model)$dw
...
pointEst <- dummy.coef(imbd_model)
...
knitr::kable(
  ...
  col.names = c("Comparison", "Difference", "Lower Bound", "Upper Bound", "Adj. p-Value", "Factor"),
  align = 'cccccc',
  booktabs = TRUE
) %>%
